...
@Component({
  selector: 'app-trade-associations',
  templateUrl: './trade-associations.page.html',
  styleUrls: ['./trade-associations.page.scss'],
})
export class TradeAssociationsPage implements OnInit {
  directory: any = [];
  id: any = [];
  org: any = [];
  pNumber: any = [];
  ...
  async ngOnInit() {
    ...
    this.taid = await this.storage.get('taid');
    this.settS();
    this.getTA();
  ...
  settS() {
    this.http
    .post('https://casa.evokemusic.net/api/trade_associations_contact_by_id', {
      id: this.taid
    })
    .subscribe((response) => {
  ...
  getTA() {
      const url = 'https://casa.evokemusic.net/api/trade_associations_contacts/';
      const f_url = `${url}${this.taid}`
