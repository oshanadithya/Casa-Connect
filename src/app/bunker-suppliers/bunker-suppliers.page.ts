...
@Component({
  selector: 'app-bunker-suppliers',
  templateUrl: './bunker-suppliers.page.html',
  styleUrls: ['./bunker-suppliers.page.scss'],
})
export class BunkerSuppliersPage implements OnInit {
  directory: any = [];
  id: any = [];
  org: any = [];
  pNumber: any = [];
  ...
  async ngOnInit() {
    ...
    this.blid = await this.storage.get('blid');
    this.setBS();
    this.getBS();
  ...
  setBS() {
    ...
    this.http
      .post('https://casa.evokemusic.net/api/bunker_lubricant_suppliers_contact_by_id', {
        id: this.blid
      })
      .subscribe((response) => {
  ...
  getBS() {
    const url = 'https://casa.evokemusic.net/api/bunker_lubricant_suppliers_contacts/';
      const f_url = `${url}${this.blid}`
